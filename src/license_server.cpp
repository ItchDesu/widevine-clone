...
std::string sign_payload(const std::string& data, EVP_PKEY* pkey) {
    ...
    // === Fuerza RSA PKCS#1 v1.5 ===
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
    EVP_PKEY_CTX* pctx = EVP_MD_CTX_pkey_ctx(ctx);
    if (pctx && EVP_PKEY_base_id(pkey) == EVP_PKEY_RSA) {
...
int main() {
    ...
    if (!svr.is_valid()) {
        std::cerr << "Failed to start server (check cert/key)\n";
        return 1;
    }
    ...
    if (!public_key_path) public_key_path = "public.pem";

    // Debug: mostrar pública que servimos
    {
        std::string pub = load_file(public_key_path);
        std::cerr << "[startup] Serving public key: "
                  << pub.substr(0, pub.find('\n')) << "\n";
    }
    ...
    svr.Post("/license", [signing_key](const httplib::Request& req, httplib::Response& res){
        ...
        catch (...) { send_error(res, 400, "invalid json"); return; }
